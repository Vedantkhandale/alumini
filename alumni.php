...
<style>
    ...
    .grid {
        ...
        position: relative;
        z-index: 5;
        /* Z-index high rakha hai visibility ke liye */
    }
    ...
    <div class="grid">
        ...
        if ($res && $res->num_rows > 0) {
            while ($row = $res->fetch_assoc()) {
                ...
                // 1. Path fix: Database me agar image name hai toh path banayein
                $imageName = $row['image'];
                $imgRelativePath = "uploads/profiles/" . $imageName;
                $imgAbsolutePath = __DIR__ . "/" . $imgRelativePath;

                // 2. Check karein ki image exist karti hai ya nahi
                $showImage = false;
                if (!empty($imageName) && file_exists($imgAbsolutePath)) {
                    $showImage = true;
                }
        ?>
                <div class="card reveal">
                    ...
                    <div class="avatar-wrapper">
                        <div class="avatar">
                            <?php if ($showImage): ?>
                                <!-- Image tag with object-fit for perfect circle/square -->
                                <img src="<?= $imgRelativePath ?>" alt="<?= htmlspecialchars($row['name']) ?>" style="width:100%; height:100%; object-fit:cover;">
                            <?php else: ?>
                                <!-- Fallback to Initial -->
                                <?= $initial ?>
                            <?php endif; ?>
